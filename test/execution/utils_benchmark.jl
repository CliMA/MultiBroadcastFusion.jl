...
"""
    device_info(device_name::String, is_cuda::Bool)

Call with `device_info(CUDA.name(CUDA.device()))`
"""
function device_info(device_name::String, is_cuda::Bool = true)
    device_specs = Dict(
...
function tabulate_benchmark(bm)
    ...
    header = [
        ...
        (allequal(problem_size) ? () : ("problem size",))...,
    ...
    args = (
        ...
        (allequal(problem_size) ? () : (problem_size,))...,
    ...
    problem_size_str =
        allequal(problem_size) ? "Problem size: $(problem_size[1]), " : ""
...
function benchmark_trial!(use_cuda, f!, X, Y)
    return if use_cuda
